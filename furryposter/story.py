...
class StoryConversionError(StoryError):
    pass


class StoryEmpty(StoryError):
    pass
...
class Story():
    ...
    def loadThumbnail(self, thumbnailProfile: str, file: BinaryIO = None):
        ...
        if file is None:
            self.forceGenThumbnail(thumbnailProfile)
        else:
            self.thumbnail = file.read()
            file.close()

    def forceGenThumbnail(self, profile: str = 'default') -> None:
        self.thumbnail = thumbnailgeneration.makeThumbnail(
            self.title, self.tags.split(', '), profile).getvalue()

    def giveStory(self, format: str) -> TextIO:
        """Returns StringIO of the story"""
        if self.content is None:
            raise StoryEmpty("Story '{}' has no content".format(self.title))
        if self.sourceFormat == format:
            return io.StringIO(self.content)
        elif self.sourceFormat == 'bbcode' and format == 'markdown':
            return io.StringIO(
                bbcodeformatter.parseStringMarkdown(
                    self.content))
        elif self.sourceFormat == 'markdown' and format == 'bbcode':
            return io.StringIO(
                markdownformatter.parseStringBBcode(
                    self.content))
        elif self.sourceFormat == 'html' and format == 'markdown':
            return io.StringIO(
                htmlformatter.formatFileMarkdown(
                    io.StringIO(
                        self.content)))
        elif self.sourceFormat == 'html' and format == 'bbcode':
            return io.StringIO(
                htmlformatter.formatFileBBcode(
                    io.StringIO(
                        self.content)))
        else:
            raise StoryConversionError(
                'Cannot convert source format {} to {}'.format(
    ...
    def giveDescription(self, format: str) -> str:
        if format == 'bbcode':
            return markdownformatter.parseStringBBcode(self.description)
        else:
